...
function JVMPin(options) {
	...
	if(!([undefined].concat(objectValues(STDIN_POLICY).some(function(x) { return x === options.stdin})))) {
		throw new Error("options.stdin must be one of " + [undefined].concat(objectValues(STDIN_POLICY)));
	}
	...
	this.spawn = function(command, args, options) {
		...
		function scheduleHeartbeat() {
			...
			if(self._heartbeatInterval) {
				setTimeout(requestHeartbeat, self._heartbeatInterval);
			}
		}
		...
		function sendChunks() {
			// Don't send anything after we've been told to exit
			if(exitChunkReceived || jvmpin_process.killed) {
				return;
			}
